...
resource "aws_instance" "rancher_server" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sleep 60",
      "docker run --rm rancher/server:${var.rancher_server_version} curl -XPUT -d '{\"value\":\"http://server.${var.domain_name}:8080\"}' -H 'Content-Type: application/json' http://${self.private_ip}:8080/v3/settings/api.host",
    ]
